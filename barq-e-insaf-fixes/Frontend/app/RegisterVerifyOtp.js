/**
 * RegisterVerifyOtp.js
 * Dedicated full-screen page shown right after "Create Account" is pressed.
 * Replaces the old in-place Modal — this is a real new page/route, as required.
 *
 * Flow:
 *  LoginScreen (Sign Up tab) → handleSignup() sends OTP, stores the pending
 *  registration payload server-side → router.push here with just { email, role }
 *  → user enters the 6-digit code (paste or type, auto-advances) →
 *  POST /auth/verify-register-otp with { email, otp } → backend creates the
 *  Supabase user row → router.replace back to Login (login tab, role kept).
 */
...
export default function RegisterVerifyOtp() {
  ...
  // Guard: this page only makes sense if we arrived with an email
  useEffect(() => {
  ...
  const handleResend = async () => {
    ...
    try {
      setLoading(true);
      // Re-send simply re-triggers the same endpoint; if the pending
      // payload already expired server-side, ask the user to go back.
