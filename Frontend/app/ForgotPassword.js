...
export default function ForgotPassword() {
  ...
  const handleSendOtp = async () => {
    ...
    if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(cleanEmail)) {
      Alert.alert('⚠️ Error', 'Sahi email address darj karein');
      return;
    }
    try {
      setLoading(true);
      ...
      Alert.alert('Error', err?.response?.data?.message || 'Kuch masla ho gaya — dobara koshish karein');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleResend = async () => {
    ...
    try {
      ...
      setCooldown(RESEND_DELAY);
      setOtpError('');
      Alert.alert('✅', 'Naya OTP email pe bhej diya gaya');
    } catch (err) {
      Alert.alert('Error', err?.response?.data?.message || 'OTP dubara nahi bheja ja saka');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleResetPassword = async () => {
    if (newPassword.length < 8) {
      Alert.alert('⚠️ Error', 'Password kam az kam 8 characters ka hona chahiye');
      return;
    }
    if (newPassword !== confirmPassword) {
      Alert.alert('⚠️ Error', 'Dono passwords ek jaise nahi hain');
      return;
    }
    try {
      setLoading(true);
      ...
      Alert.alert('Error', err?.response?.data?.message || 'Password reset fail — dobara koshish karein');
    } finally {
      setLoading(false);
    }
  };
