...
export default function LoginScreen() {
  ...
  const handleGoogleSuccess = async (data) => {
    try {
      ...
      showAlert('Error', 'Login succeeded but failed to save session');
    }
  };
  ...
  const handleSignup = async () => {
    ...
    if (Object.keys(newErrors).length > 0) {
      ...
      showAlert('Form Error ⚠️', firstErrMsg || 'Please fix the highlighted fields to continue.');
      return;
    }
          ...
          {activeTab === 'login' && (
            <>
              ...
              <GoogleLoginButton
                ...
                onError={(msg) => showAlert('Google Login Error', msg)}
              />
            </>
          )}
